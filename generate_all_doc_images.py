...
def _save_points(pts: pg.SetPoints, outdir: Path, stem: str, *, figsize=(8, 8), **draw_kwargs) -> Path:
    _ensure_dir(outdir)
    ret = pts.draw(save=str(outdir / stem), figsize=figsize, **draw_kwargs)
    _close_fig(_unwrap_fig(ret))
...
def _save_graph_single(graph: object, outdir: Path, stem: str, *, figsize=(8, 8), **draw_kwargs) -> Path:
    _ensure_dir(outdir)
    try:
        ret = graph.draw(save=str(outdir / stem), figsize=figsize, **draw_kwargs)
        _close_fig(_unwrap_fig(ret))
        return outdir / f"{stem}.png"
    except TypeError:
        pass

    ret = graph.draw(figsize=figsize, **draw_kwargs)
    fig = _unwrap_fig(ret)
    if fig is None:
...
def _save_graph_grid(graphs: List[object], outdir: Path, stem: str, nrows: int, ncols: int, *, figsize=(10, 10), **draw_kwargs) -> Path:
    ...
    try:
        ret = pg.draw_grid(graphs, nrows, ncols, figsize=figsize, **draw_kwargs)
        fig = _unwrap_fig(ret)
        if fig is None:
...
def _points_jobs(points_dir: Path) -> List[Job]:
    ...
    def _transformations() -> None:
        ...
        fig.savefig(outdir / "transformations.png", dpi=200, bbox_inches="tight")
        plt.close(fig)
