...
1. The number of points N is drawn from a Poisson distribution with mean L = intensity × length, where length = 2pir (circumference).
2. Given N points, their angular positions theta are drawn independently from U(0, 2pi).
3. Convert to Cartesian coordinates: x = r cos(theta), y = r sin(theta).
