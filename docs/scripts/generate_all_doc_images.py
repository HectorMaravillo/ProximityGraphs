...
def _image_path(outdir: Path, stem: str, *, ext: str = IMAGE_EXT) -> Path:
...
def _save_points(pts: pg.SetPoints, outdir: Path, stem: str, *, figsize=(8, 8), **draw_kwargs) -> Path:
    ...
    out_path = _image_path(outdir, stem)
    _save_figure(fig, out_path)
    _close_fig(fig)
    return out_path


def _save_graph_single(graph: object, outdir: Path, stem: str, *, figsize=(8, 8), image_ext: str = IMAGE_EXT, **draw_kwargs) -> Path:
    _ensure_dir(outdir)
    ...
    out_path = _image_path(outdir, stem, ext=image_ext)
    _save_figure(fig, out_path)
    _close_fig(fig)
    return out_path
...
def _save_graph_grid(graphs: List[object], outdir: Path, stem: str, nrows: int, ncols: int, *, figsize=(10, 10), **draw_kwargs) -> Path:
    ...
    for i, g in enumerate(graphs):
        ...
        tmp_png = _image_path(outdir, tmp_stem, ext="png")
        _save_graph_single(g, outdir, tmp_stem, figsize=figsize, image_ext="png", **draw_kwargs)
...
def _points_jobs(points_dir: Path) -> List[Job]:
    ...
    def _transformations() -> None:
        ...
        _save_figure(fig, _image_path(outdir, "transformations"))
        plt.close(fig)
