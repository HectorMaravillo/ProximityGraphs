...
class ProximityGraph(GeometricGraph):
    ...
    def __check_parameter(cls, parameter,
        ...
        """
        Validates a parameter against a specified type and range.

        This is a utility method to ensure that parameters for graph
        ...
        range_min : int or float, optional
            The minimum allowed value for the parameter (inclusive).
        range_max : int or float, optional
            The maximum allowed value for the parameter (inclusive).
        data_type : list, optional
            A list of allowed types for the parameter.

        Raises
        ------
        TypeError
            If the parameter is not of a valid type.
        ValueError
            If the parameter is outside the specified range.
...
class Beta_Skeleton(ProximityGraph):
    """
    Constructs the β-skeleton of a set of points.

    The β-skeleton is a proximity graph where two points p and q are connected
    if their "neighborhood" is empty of any other points from the set. The
    definition of this neighborhood depends on the parameter β.

    For β ≥ 1, the neighborhood is the intersection of two circles of radius
    (β/2) * d(p, q), where d(p, q) is the Euclidean distance between p and q.
    The centers of these circles are located on the line segment connecting p
    and q. This region is often called a "lune". An edge (p, q) exists if
    this lune contains no other points.

    - If β = 1, the region is the intersection of two circles with diameter
      d(p, q), and the resulting graph is the Gabriel Graph.
    - As β increases, the lune becomes thinner, resulting in more edges.

    This implementation focuses on the lune-based definition for β ≥ 1 and
    leverages the Delaunay triangulation to reduce the number of candidate
    edges, since the β-skeleton is a subgraph of the Delaunay triangulation
    for β ≥ 1.

    Attributes
    ----------
    name : str
        The name of the graph, set to "β-Skeleton".
    details : str
        Additional details about the graph, including the value of β.
    ...
    def __init__(self, setpoints, beta=1.5, type_region="lune", closed=False):
        ...
        setpoints : SetPoints
            An object containing the set of points.
        beta : float, optional
            The parameter defining the shape of the empty region (default is 1.5).
            Must be non-negative.
        type_region : str, optional
            The type of empty region to use. Currently, only "lune" is
            supported, which is valid for β ≥ 1 (default is "lune").
        closed : bool, optional
            Whether the empty region is closed (includes boundary). Currently
            this parameter is not fully utilized in the edge assignment logic
            but is kept for future extensions (default is False).
        ...
        if beta >= 1:
            # For beta >= 1, the skeleton is a subgraph of the Delaunay graph.
            g_delaunay = DelaunayG(setpoints)
            pairs = np.array(g_delaunay.graph.get_edgelist())
            if type_region == "lune":
                # Defines the function to get the centers of the lune's circles.
                empty_region = lambda p, q: self.__lune(p, q, beta)
                # Defines the test for a point being inside the lune.
                self.__test = lambda test_1, test_2: test_1 * test_2
            else:
                raise ValueError("Only 'lune' type_region is supported for β >= 1")
        else:
            # For 0 <= beta < 1, all pairs must be checked.
            pairs = np.array(list(combinations(range(self.n), 2)))
            # Here you would define the empty region for beta < 1, if supported.
            raise NotImplementedError("β-skeleton for β < 1 is not implemented.")
    ...
    def from_graph(cls, geom_graph, beta=1.5, closed=False):
        """
        Creates a Beta_Skeleton from an existing GeometricGraph.

        This method is useful for creating a β-skeleton from a pre-computed
        graph, such as a Delaunay triangulation, to reduce computations.
        ...
        beta : float, optional
            The β parameter for the skeleton (default is 1.5).
        closed : bool, optional
            Whether the empty region is closed (default is False).

        Returns
        -------
        Beta_Skeleton
            A new Beta_Skeleton object.
    ...
    def __assign_edges(self, pairs, beta):
        """
        Tests pairs of points and adds edges to the graph if the empty region
        criterion is met.

        For each pair of points (p, q), it constructs the lune-based empty
        region and checks if any other point from the set falls inside it.

        Parameters
        ----------
        pairs : array-like
            An array of point index pairs to be tested.
        beta : float
            The β parameter.
    ...
    def __lune(cls, p, q, beta):
        """
        Calculates the centers of the two circles forming the lune for β ≥ 1.

        The centers are computed as affine combinations of p and q.
        Center 1: c1 = (1 - β/2)p + (β/2)q
        Center 2: c2 = (β/2)p + (1 - β/2)q
        This formula is slightly different from the code but mathematically equivalent.
        The code uses:
        c1 = p*beta_aux + (1-beta_aux)*q, with beta_aux = beta/2.
           = p*beta/2 + q - q*beta/2 = q + (p-q)*beta/2
        c2 = q*beta_aux + (1-beta_aux)*p
           = q*beta/2 + p - p*beta/2 = p + (q-p)*beta/2
        These are the correct centers for the circles.
        ...
        -------
        tuple
            A tuple containing the coordinates of the two circle centers.
