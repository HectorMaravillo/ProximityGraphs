"""Fungal-inspired constructive graph construction.

This module implements a mycelium-like network heuristic. The graph starts
from a Physarum-inspired backbone and then grows additional short, useful
connections subject to degree constraints before optionally pruning weaker
non-backbone edges.

The method is not a direct reproduction of a single biological experiment; it
is a constructive bio-inspired model that combines adaptive-network ideas with
sparse proximity-graph principles to obtain connected, efficient networks.
    ...
    def _initialize_network(
        ...
        for i in range(n):
            for j in range(i + 1, n):
                if dist_matrix[i, j] <= threshold_dist and not self.graph.are_adjacent(
                    i, j
                ):
